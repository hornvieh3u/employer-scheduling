...
const baseUrl = process.env.REACT_APP_BASE_UR;
...
export const LOGIN_WITH_JWT = ({ username, password }) => {
  return async (dispatch) => {
    try {
      let response = await axios.post(`${baseUrl}/api/signin`, {
        username,
        password
      });
...
export const LOGING_MULTIPLE_LOCATION = (payload) => {
  return async (dispatch) => {
    try {
      let response = await axios.post(`${baseUrl}/api/signin`, payload);
...
export const Get_User_Info = () => {
  return async (dispatch) => {
    try {
      let response = await axios.get(`${baseUrl}/api/organization_setup_info/${getUserId()}`, {
        headers: {}
      });
      if (response.data && response.status === 200) {
        dispatch({
...
export const UPDATE_RATING_FOR_SCHOOL = () => {
  ...
  return async (dispatch) => {
    try {
      await axios.get(urlRating, { headers: getHeaders() });
    } catch (error) {
      toast.error(error.message.replace(/\\/g, ''), toastCSS());
    }
  };
