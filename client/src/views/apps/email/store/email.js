...
const domain = process.env.REACT_APP_API.replace('/api/', '') //"http://localhost:5000"
const baseUrl2 = process.env.REACT_APP_API.replace('/api/', '') //"http://localhost:5000"
...
export const GET_SENT_EMAILS = () => {
  let url = `${domain}/api/all_sent/${getUserId()}`;
...
export const GET_CATEGORIES_EMAIL = (urlPath) => {
  let url = `${domain + urlPath}/category_list/${getUserId()}`;
  return async (dispatch) => {
    try {
...
export const GET_ALL_SECHEDULE_EMAIL = () => {
  let url = `${domain}/api/scheduleListing/${getUserId()}`;
  return async (dispatch) => {
    try {
      let response = await API.get(url, { headers: getHeaders() });
      if (response.data && response.status === 200) {
...
export const ADD_NEW_MAIN_FOLDER_EMAIL = (urlPath, data) => {
  let url = `${domain + urlPath}/addCategory/${getUserId()}`;
  return async (dispatch) => {
    try {
...
export const VERIFY_NEW_EMAIL_ADDRESS = (payload) => {
  let url = `${domain}/api/emailsendgrid/sendverification/${getUserId()}`;
  return async (dispatch) => {
    try {
...
export const GET_ACTIVE_OR_INACTIVE_LIST = () => {
  let url = `${domain}/api/getsendgridverification/${getUserId()}`;
  return async (dispatch) => {
    try {
      let response = await API.get(url, { headers: getHeaders() });
      if (response?.data?.success) {
...
export const DELETE_EMAIL_FROM_VERIFICATION_LIST = (email) => {
  let url = `${domain}/api/delete/verifiedsendgriduser/${getUserId()}/${email?.toLowerCase()}`;
  return async (dispatch) => {
    try {
...
export const SEND_LINK_FOR_VERIFICATION = (payload) => {
  let url = `${domain}/api/sendgridverification/emails/${getUserId()}`;
  return async (dispatch) => {
    try {
...
export const ADD_NEW_SUB_FOLDER_EMAIL = (urlPath, data, folderId) => {
  let url = `${domain + urlPath}/create_folder/${getUserId()}/${folderId}`;
  return async (dispatch) => {
    try {
      let response = await API.post(url, data, { headers: getHeaders() });
      if (response.data && response.status === 200) {
...
export const DRAG_DROP_TEMPLETE_EMAIL_IN_ORDER = (urlPath, payload) => {
  let url = `${domain + urlPath}/drag_drop_templete/${getUserId()}`;
  return async (dispatch) => {
    try {
...
export const SWAP_TEMPLATE = (folderID, payload) => {
  let url = `${domain}/api/email_nurturing/swap_templete/${getUserId()}/${folderID}`;
  return async (dispatch) => {
    try {
      await API.put(url, payload, { headers: getHeaders() });
    } catch (error) {
...
export const SEND_EMAIL = (Urlpath, payload) => {
  let url = `${domain}${Urlpath}/send_email/${getUserId()}`;
  return async (dispatch) => {
    try {
...
export const ADD_TEMPLATE_TO_EMAIL = (Urlpath, data, id) => {
  let url = `${domain}${Urlpath}/add_template/${getUserId()}/${id}`;
  return async (dispatch) => {
    try {
...
export const UPDATE_TEMPLATE_TO_EMAIL = (UrlPath, payload, templateId, FId) => {
  let url = `${domain}${UrlPath}/update_template/${getUserId()}/${templateId}`;
  return async (dispatch) => {
    try {
...
export const MAKE_TEMPLATE_AS_FAVORITES = (UrlPath, payload, templateId, FId) => {
  let url = `${domain}${UrlPath}/marks_as_star/${getUserId()}/${templateId}`;
  return async (dispatch) => {
    try {
      let response = await API.put(url, payload, {
        headers: getHeaders()
...
export const MAKE_TEMPLATE_AS_ACTIVATE = (UrlPath, payload, FId) => {
  let url = `${domain}/api/activate_mail/${getUserId()}`;
  return async (dispatch) => {
    try {
      let response = await API.put(url, payload, {
        headers: getHeaders()
...
export const DELETE_MULTIPLE_TEMPLATE = (Urlpath, data, id) => {
  let url = `${domain}${Urlpath}/multipal_remove_template/${getUserId()}/${id}`;
  return async (dispatch) => {
    try {
...
export const DELETE_CATEGORY_EMAIL = (UrlPath, folderId) => {
  let url = `${domain + UrlPath}/remove_category/${getUserId()}/${folderId}`;
  return async (dispatch) => {
    try {
      let response = await API.delete(url, { headers: getHeaders() });
      if (response.data) {
...
export const DELETE_SUB_FOLDER_EMAIL = (UrlPath, folderId) => {
  let url = `${domain + UrlPath}/delete_folder/${getUserId()}/${folderId}`;
  return async (dispatch) => {
    try {
      let response = await API.delete(url, { headers: getHeaders() });
      if (response.data) {
        dispatch(GET_CATEGORIES_EMAIL());
...
export const GET_SCHEDULE_MAILS = (UrlPath, folderId) => {
  let url = `${domain + UrlPath}/list_template/${getUserId()}/${folderId}`;
  return async (dispatch) => {
    try {
      let response = await API.get(url, { headers: getHeaders() });
      if (response.data && response.status === 200) {
...
export const DELETE_SCHEDULE_MAIL = (urlpath, templateId) => {
  let url = `${domain + urlpath}/remove_template/${getUserId()}/${templateId}`;
  return async (dispatch) => {
    try {
      let response = await API.delete(url, { headers: getHeaders() });
      if (response.data.success) {
...
export const UPDATE_EMAIL_CATEGORY = (urlpath, folderId, folderName, type) => {
  ...
  let url = `${domain + urlpath}/${uriName}/${getUserId()}/${folderId}`;
  return async (dispatch) => {
    try {
      let response = await API.put(url, payload, { headers: getHeaders() });
      if (response.data && response.status === 200) {
...
export const GET_ALL_SMART_LIST = () => {
  ...
  if (userData.role === 'admin') {
    url = `${baseUrl2}/api/admin/smartlists/folder_list/${getUserId()}`;
  } else {
    url = `${baseUrl2}/api/smartlists/folder_list/${getUserId()}`;
  }
  url = `${baseUrl2}/api/smartlists/folder_list/${getUserId()}`;
  return async (dispatch) => {
    try {
...
export const UPDATE_SMART_LIST = (payload, id) => {
  let url = '';
  if (userData.role === 'admin') {
    url = `${baseUrl2}/api/admin/smartlists/update_smartlist/${getUserId()}/${id}`;
  } else {
    url = `${baseUrl2}/api/smartlists/update_smartlist/${getUserId()}/${id}`;
  }
  return async (dispatch) => {
    try {
      let response = await API.put(url, payload, { headers: getHeaders() });
...
export const DELETE_SMART_LIST = (id) => {
  let url = '';
  if (userData.role === 'admin') {
    url = `${baseUrl2}/api/admin/smartlists/delete_smartlist/${getUserId()}/${id}`;
  } else {
    url = `${baseUrl2}/api/smartlists/delete_smartlist/${getUserId()}/${id}`;
  }
  return async (dispatch) => {
    try {
      let response = await API.delete(url, { headers: getHeaders() });
...
export const UPDATE_SMART_LIST_FOLDER = (payload, id) => {
  let url = '';
  if (userData.role === 'admin') {
    url = `${baseUrl2}/api/admin/smartlists/update_Folder/${getUserId()}/${id}`;
  } else {
    url = `${baseUrl2}/api/smartlists/update_Folder/${getUserId()}/${id}`;
  }
  return async (dispatch) => {
    try {
      let response = await API.put(url, payload, { headers: getHeaders() });
...
export const DELETE_SMART_LIST_FOLDER = (id) => {
  let url = '';
  if (userData.role === 'admin') {
    url = `${baseUrl2}/api/admin/smartlists/delete_Folder/${getUserId()}/${id}`;
  } else {
    url = `${baseUrl2}/api/smartlists/delete_Folder/${getUserId()}/${id}`;
  }
  return async (dispatch) => {
    try {
      let response = await API.delete(url, { headers: getHeaders() });
...
export const CREATE_SMART_LIST_FOLDER = (payload) => {
  let url = '';
  if (userData.role === 'admin') {
    url = `${baseUrl2}/api/admin/smartlists/createFolder/${getUserId()}`;
  } else {
    url = `${baseUrl2}/api/smartlists/createFolder/${getUserId()}`;
  }
  return async (dispatch) => {
    try {
      let res = await API.post(url, payload, {
...
export const CREATE_SMAART_LIST = (payload, folderId) => {
  let url = '';
  if (userData.role === 'admin') {
    url = `${baseUrl2}/api/admin/smartlists/create_smartlist/${getUserId()}/${folderId}`;
  } else {
    url = `${baseUrl2}/api/smartlists/create_smartlist/${getUserId()}/${folderId}`;
  }
  return async (dispatch) => {
    try {
      let res = await API.post(url, payload, {
...
export const ADD_CRITERIA_MET = (urlPath, data) => {
  let url = `${domain}/api/criteria_met/email_compose/add_template/${getUserId()}`;
  return async (dispatch) => {
    try {
      let response = await API.post(url, data, { headers: getHeaders() });
      if (response.data && response.status === 200) {
...
export const UPLAODE_IMAGE = (data) => {
  let url = `${domain}/api/email/eventManager`;
  return async (dispatch) => {
    try {
