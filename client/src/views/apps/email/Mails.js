...
import { Menu, Search, Folder, Tag, Mail, Trash, Edit2, Info, RotateCcw } from 'react-feather';
...
const Mails = (props) => {
  ...
  const handleMailClick = (mail) => {
    if (mail.status.includes('draft')) {
      toggleCompose('edit', mail);
    } else {
      dispatch(selectCurrentMail(mail._id));
      setOpenMail(true);
    }
  };
  ...
  const handleMailToTrash = (ids, isTrash = false) => {
    dispatch(
      updateMails({
        emailIds: ids,
  ...
  const handleMailSendNow = (ids) => {
    dispatch(
      updateMails({
        emailIds: ids,
        dataToUpdate: { folder: 'send-now' }
      })
    );
    dispatch(resetSelectedMail());
  };
  ...
  return (
    <Fragment>
              ...
              <ul className="list-inline m-0">
                ...
                {currentFolder === 'trash' ? (
                  <li className="list-inline-item">
                    <span
                      className="action-icon"
                      onClick={() => handleMailToTrash(selectedMails, true)}
                    >
                      <RotateCcw size={18} />
                    </span>
                  </li>
      ...
      <MailDetails
        ...
        handleMailSendNow={handleMailSendNow}
      />
      <ComposePopUp
