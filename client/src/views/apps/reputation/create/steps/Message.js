...
import { blobToBase64, convertDocx, createImageDoc,convertBase64 } from '../../helpers/loadPdfHelper'
        ...
        if (documentFiles.length > 0) {
            ...
            if (recipients.length > 0) {
                if (isOnlySigner) {
                    ...
                    const temp = recipients.some(
                        (r) => r.name === '' || r.email === ''
                    )

                    if (temp) {
                        toast.error(
                            'Name and email is required for all recipients'
                        )
                    } else {
                        const regex =
                            /^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i
                        const validEmail = recipients.filter((r) =>
                            regex.test(r.email)
                        )
     ...
     setIsUploading(true)
        ...
        for (const file of documentFiles) {
            if (file.type.startsWith('image')) {
                ...
                const copiedPdfPages = await mergedDoc.copyPages(
                    pdfFile,
