...
function AddAutomationModal(props) {
  ...
  const handleFieldValue = (e) => {
    ...
    } else setTemplatePayload({ ...templatePayload, [name]: value });
  };
    ...
    if (templatePayload?.adminId !== undefined) {
      ...
      if (send) {
        ...
        if (template) {
          ...
          ADD_TEMPLATE_TO_EMAIL(`/api/email_nurturing`, formData, folderId);
          setEditOrAddOrListTemplate('list');
        }
      }
    } else {
      ...
      if (!payload.sent_date) {
        payload = {
          ...
          sent_time: moment(new Date()).format('HH:mm')
        };
      }
  ...
  const handleSubmit = () => {
    ...
    // })
    AddNewTemplate(props.subFolderActive);
  };
  ...
  return (
    <div>
      <div className="d-flex" onClick={toggleCompose}>
        <img
          src={type == 'email' ? sendEmailSvg : sendTextSvg}
          style={{ width: '36px', height: '36px', marginRight: '15px', objectFit: 'contain' }}
        />
        <div>
          ...
          <div>
            {' '}
            {type == 'email'
              ? 'Create a text message to be sent to your client.'
              : 'Create a text message to be sent to your client.'}
          </div>
        </div>
      </div>
