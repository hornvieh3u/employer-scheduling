...
import { mergeDocument } from '../store/api';
    ...
    <Modal isOpen={isMergeModalOpen} className="modal-dialog-centered modal-xl">
      ...
      <ModalBody>
        <div className="d-flex align-items-center mt-2 ml-2">
          ...
          <h5 className="ms-1 mb-0">MyDrive{currentPath}</h5>
        </div>
        <AddItem />
          ...
          {folders.length || files.length ? (
            <ul className="todo-task-list bg-white p-2">
              {folders.concat(files).map((item) => {
                return (
                  <ListItem isMerge={true} item={item} setIsMergeModalOpen={setIsMergeModalOpen} />
                );
              })}
            </ul>
          ) : (
        ...
        {/* <div className="d-flex justify-content-between mt-2">
          ...
          <Button color="primary" className="btn-next" onClick={() => handleMerge()}>
