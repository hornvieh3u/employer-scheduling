...
// import client positions from 
import { useGetRelationContacts, useGetRelationPosition } from '../../../../requests/contacts/relationship-contacts'
...
const CustomHeader = ({
    ...
    const csvReport = {
        filename: "relationship-contacts.csv",
        headers: headers,
        data: tableData
    }
    ...
    const downloadPdf = () => {
        const doc = new jsPDF()
        doc.text("Relationship Contacts", 15, 10)
        doc.autoTable({
            styles: {
                fontSize: 8
            },
        ...
        doc.save('relationship-contacts.pdf')
    }
...
const LeadsList = () => {
    ...
    const { data: tableData, refetch: relationRefetch } = useGetRelationContacts()

    // ** Store Vars
    const dispatch = useDispatch()
    const store = useSelector((state) => state.users)
    const { contactList } = useSelector((state) => state.relationshipContact)
    ...
    // Default Relationship type roles
    const roleOptions = [
    ...
    const { contactUpload } = useSelector((state) => state.relationshipContact)
                ...
                <CardBody>
                    <Row>
                        <Col className="my-md-0 my-1" md="6">
                            <Label for="plan-select">Relationship Type</Label>
                            <Select
            ...
            {1 === 3 && (
                <Card>
                    ...
                    <CardBody>
                        <Row>
                            <Col md="4">
                                <Label for="role-select">Contact Type</Label>
                                <Select
                                    isClearable={false}
                                    value={currentRole}
                                    options={roleOptions}
