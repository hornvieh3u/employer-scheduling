...
import { addcustomer, fetchcustomerdata } from '../../../../requests/invoice/invoice'

const EditCard = ({ invoicedata, setinvoice }) => {
    // ** States
    ...
    useEffect(() => {
        ...
        const arr = options
        data?.data?.map((item) =>
            arr.push({ value: item._id, label: item.fullName })
        )
        setOptions([...arr])
    }, [data?.data])
    ...
    const handleInvoiceToChange = (value) => {
        setValue(value)
        setinvoice({
            ...invoicedata, customerId: value?.value
        })
        setSelected(data?.data?.filter((i) => i._id === value?.value)[0])
    }
    // add customer
    const handlesubmit = () => {
    ...
    const handlescustomerdatachange = (e) => {
        const { name, value } = e.target
        if (name === "street") {
            const invoice = { ...addcustomerdata?.address }
            invoice[name] = value
            setaddcustomerdata({
                ...addcustomerdata,
                address: invoice
                                ...
                                {data?.data !== null ? (
                                    <Fragment>
                                        ...
                                        <CardText className="mb-25">
                                            {selected?.fullName || invoicedata.customer[0]?.company}                                        </CardText>
                                        <CardText className="mb-25">
                                            {  selected?.address?.street || invoicedata.customer[0]?.address?.street}
                                        </CardText>
                                        <CardText className="mb-25">
                                            {selected?.contact || invoicedata.customer[0]?.contact}
                                        </CardText>
                                        <CardText className="mb-0">
                                            {selected?.email || invoicedata.customer[0]?.email}
                                        </CardText>
                                    </Fragment>
                                ) : null}
                            </div>
                                        ...
                                        <Row className="w-100 pe-lg-0 pe-1 py-2">
                                            ...
                                            >
                                                ...
                                                <Input
                                                    ...
                                                    onChange={(e) => { handlechnageitems(e, i) }}
                                                    name="item"
                                                >
                                                    <option>
                                                        App Design
                        ...
                        >
                            <div className="d-flex align-items-center mb-1">
                                ...
                                <Input
                                    ...
                                    defaultValue={invoicedata?.salesperson}
                ...
                <Form>
                    <div className="mb-2">
                        ...
                        <Input
                            ...
                            name='fullName'
                            onChange={(e) => { handlescustomerdatachange(e) }}
                            required
                        />
                    </div>
                    <div className="mb-2">
