...
const LeadPosition = require("../models/LeadPosition");
...
const getContacts = asyncHandler(async (req, res) => {
  try {
    ...
    const leadContacts = await LeadContact.aggregate([
      {
        $match: query,
      },
      {
...
const contactAdd = asyncHandler(async (req, res) => {
  const { fullName, email, phone, type, company, position, country, stage } = req.body;
...
const deleteContact = asyncHandler(async (req, res) => {
  ...
  const leadContact = await LeadContact.findById(_id);
...
const uploadAvatar = async (req, res) => {
  try {
    ...
    const lead = await LeadContact.findById(id);
    if (!lead) {
...
const updateSocialLink = async (req, res) => {
  try {
    ...
    const employee = await LeadContact.findById(id);
    if (!employee) {
      return res.status(404).send("ID not found ");
    }
...
const fileAddAndUpdate = async (req, res) => {
  try {
    ...
    const employee = await LeadContact.findById(employeeId);
    if (!employee) {
      return res.status(404).send("ID not found ");
    }
...
async function totalLeads(req, res) {
  try {
    const contacts = await LeadContact.find({
      userId: req.user._id,
      isDelete: false,
    });
...
async function totalLeadsCount(req, res) {
  try {
    const contactsCount = await LeadContact.countDocuments({
      userId: req.user._id,
      isDelete: false,
    });
    // console.log("total", contactsCount);
...
async function ColdLeadCount(req, res) {
  try {
    const contacts = await LeadContact.countDocuments({
      stage: "cold",
      userId: req.user._id,
      isDelete: false,
    });
...
async function warmLeadCount(req, res) {
  try {
    const employees = await LeadContact.countDocuments({
      stage: "warm",
      userId: req.user._id,
      isDelete: false,
    });
...
const updateContactStage = async (req, res) => {
  try {
    const client = await LeadContact.findById(req.body.id);
    client.stage = req.body.newStage;
    await client.save();
...
const updateContact = (req, res) => {
  ...
  LeadContact.findOne({ _id, isDelete: false }, (err, leadContact) => {
    if (err) {
      return res.status(500).json({
        errors: { common: { msg: err.message } },
      });
    ...
    if (!leadContact) {
      ...
      leadContact.stage = stage ? stage : leadContact.stage;
...
const addNewOther = async (req, res) => {
  try {
    ...
    const lead = await LeadContact.findOne({
      _id: clientId,
      isDelete: false,
    });
...
const editOther = async (req, res) => {
  try {
    ...
    const client = await LeadContact.findOne({
      _id: clientId,
      isDelete: false,
    });
...
const removeOther = async (req, res) => {
  try {
    ...
    let client = await LeadContact.findOne({
      _id: leadContact,
      isDelete: false,
    });
...
const importContactsFromArray = async (req, res) => {
  try {
    ...
    await LeadContact.insertMany(formatedData);
...
const leadPosition = asyncHandler(async (req, res) => {
  const { position } = req.body;

  const leadPosition = new LeadPosition({
    userId: req.user.id ? req.user.id : req.user._id,
    position,
  });
  ...
  leadPosition.save((err, success) => {
    if (err) {
      ...
      return res.status(201).json({
        success: "Lead contact created successfull",
      });
    }
  });
});
...
const getLeadPositions = async (req, res) => {
  try {
    const { user } = req;
    const leadPosition = await LeadPosition.find({
      userId: user._id,
    });

    return res.status(200).send(leadPosition);
  } catch (err) {
    return res.status(500).json({
      errors: { common: { msg: err.message } },
    });
...
const deleteLeadPosition = async (req, res) => {
  try {
    const { id } = req.params;
    await LeadPosition.deleteOne({ _id: id });
...
const putLeadPosition = async (req, res) => {
  try {
    const { id } = req.params;
    const { position } = req.body;
    const filter = { _id: id };
    ...
    await LeadPosition.updateOne(filter, updatedDoc, options);
